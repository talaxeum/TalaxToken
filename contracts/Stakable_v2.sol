...
contract Staking is ReentrancyGuard {
    ...
    mapping(address => uint256) public s_balances; // user -> how much they staked
    ...
    modifier updateReward(address account) {
        s_rewardPerTokenStored = rewardPerToken(); // new reward per token
        s_lastUpdateTime = block.timestamp;
        s_rewards[account] = earned(account);
        s_userRewardPerTokenPaid[account] = s_rewardPerTokenStored; // latest reward per token
        _;
    }
